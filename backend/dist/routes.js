...
function parseInitData(messenger, initData) {
    ...
    if (messenger === 'TELEGRAM') {
        ...
        const isValid = (0, validation_1.validateTelegramData)(initData, token);
        if (!isValid && !isDev) {
        ...
        if (!userStr)
            throw new Error('User data missing in Telegram initData');
        ...
        const isValid = (0, validation_1.validateMAXData)(initData, token);
        if (!isValid && !isDev) {
        ...
        if (!userStr)
            throw new Error('User data missing in MAX initData');
        const user = JSON.parse(userStr);
        return {
            userId: String(user.id),
            firstName: user.first_name || '',
...
router.get('/points', async (req, res) => {
    try {
        const points = await db_1.default.pickupPoint.findMany();
        res.json(points);
    }
    catch (error) {
...
router.get('/points/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const point = await db_1.default.pickupPoint.findUnique({
            where: { id },
        });
        if (!point) {
...
router.get('/menu', async (req, res) => {
    try {
        ...
        const dailyStocks = await db_1.default.dailyStock.findMany({
            where: {
                date: {
                    gte: date,
...
router.post('/orders', async (req, res) => {
    try {
        const { messenger, initData, clientName, clientPhone, clientComment, pickupPointId, slotDate, // YYYY-MM-DD
...
router.post('/admin/points', async (req, res) => {
    try {
        ...
        const newPoint = await db_1.default.pickupPoint.create({
            data: {
                name,
...
router.post('/admin/orders/:id/status', async (req, res) => {
    try {
        ...
        if (messageText && order.clientTgId) {
            await (0, bot_1.sendNotification)(order.messenger, order.clientTgId, messageText);
        }
...
router.post('/admin/stock', async (req, res) => {
    try {
        ...
        const existingStock = await db_1.default.dailyStock.findFirst({
            where: {
                menuItemId,
                date: {
                    gte: d,
        ...
        if (existingStock) {
            ...
            const updated = await db_1.default.dailyStock.update({
                where: { id: existingStock.id },
                data: { totalCount: parsedTotalCount },
            });
            return res.json(updated);
        }
        const newStock = await db_1.default.dailyStock.create({
            data: {
                menuItemId,
                date: d,
                slotStart,
 ...
 */
router.post('/admin/orders/:id/courier', async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.post('/admin/orders/:id/items', async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/hubs', async (req, res) => {
    try {
        const hubs = await db_1.default.hub.findMany();
        res.json(hubs);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.get('/logistics/tasks', async (req, res) => {
    try {
        const tasks = await db_1.default.logisticsTask.findMany({
            include: {
                order: {
                    include: {
 ...
 */
router.post('/logistics/tasks/batch', async (req, res) => {
    try {
        const { driverName, tasks } = req.body;
        if (!tasks || !Array.isArray(tasks) || tasks.length === 0) {
            return res.status(400).json({ error: 'Tasks array is required' });
        }
        const createdTasks = await Promise.all(tasks.map((t) => db_1.default.logisticsTask.create({
            data: {
 ...
 */
router.put('/logistics/tasks/:id/status', async (req, res) => {
    try {
        const { id } = req.params;
        const { status } = req.body;
        const task = await db_1.default.logisticsTask.update({
            where: { id },
            data: { status },
            include: { order: true }
        });
...
router.post('/admin/menu', async (req, res) => {
    try {
        ...
        const newItem = await db_1.default.menuItem.create({
            data: {
